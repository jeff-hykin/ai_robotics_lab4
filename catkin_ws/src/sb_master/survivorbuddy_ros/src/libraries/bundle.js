...
function subtract({ value , from: from1  }) {
    let source = from1;
    let detractor = value;
    ...
    if (sourceSize < detractorSize) {
        ...
        !(source != from1) && (source = new Set(source));
...
var e = "", t = "";
function o(r1) {
    var p1, a1, l2, s, c1 = arguments, i11 = this, n = 0, d = [], h = 0, u = [], f = 0;
    d.root = !0;
...
createCssClass(`checkbox`, [
    `{
                box-sizing: border-box;
                padding: 0;
            }`, 
]);
Symbol("originalDisplayValue");
createCssClass(`dropdownPlaceholder`, [
    `{
                overflow: visible;
            }`, 
]);
createCssClass(`dropdownList`, [
    `{
                overflow: auto;
                height: fit-content;
                max-height: 50vh;
...
function eightToSeven(eightBytes) {
    const seven = 7;
    const sevenBytes = eightBytes.slice(0, 7);
    const finalByte = eightBytes[7];
    const newBytes = new Uint8Array(new ArrayBuffer(7));
    let index = -1;
...
function stringToBytes(string) {
    ...
    const numberOfBlocks = Math.ceil(chunksOfEight.length / 8);
    const arrays = [];
    for(let index in [
        ...Array(numberOfBlocks)
    ]){
        index -= 0;
        arrays.push(eightToSeven(chunksOfEight.slice(index * 8, (index + 1) * 8)));
    }
...
Object.getOwnPropertyDescriptors;
const allKeyDescriptions1 = function(value, options = {
    includingBuiltin: false
}) {
    var { includingBuiltin  } = {
        ...options
...
Object.getPrototypeOf(new Map().keys());
Object.getPrototypeOf(new Set().keys());
let GeneratorFunction1 = class {
};
let AsyncGeneratorFunction1 = class {
};
try {
    GeneratorFunction1 = eval("((function*(){})()).constructor");
    AsyncGeneratorFunction1 = eval("((async function*(){})()).constructor");
} catch (error4) {}
Symbol.for("deepCopy");
Symbol();
const getThis1 = Symbol();
Object.getPrototypeOf(function() {})[getThis1] = function() {
    return this;
};
var e1 = "", t1 = "";
function o1(r1) {
    var p1, a1, l3, s, c1 = arguments, i110 = this, n = 0, d = [], h = 0, u = [], f = 0;
    d.root = !0;
    var g = function(e11, o11, r2) {
        ...
        [d, s, ...p1] = d, d.push(i110(s, ...p1));
    };
    return r1.join(e1).replace(/<!--[^]*-->/g, "").replace(/<!\[CDATA\[[^]*\]\]>/g, "").replace(/('|")[^\1]*?\1/g, (e2)=>(u.push(e2), t1)
    ).replace(/\s+/g, " ").replace(/(?:^|>)([^<]*)(?:$|<)/g, (e3, t11, r4, p3)=>{
        var c, i21;
        if (r4 && p3.slice(n, r4).replace(/(\S)\/$/, "$1 /").split(" ").map((e4, t2)=>{
            if ("/" === e4[0]) c = i21 || e4.slice(1) || 1;
            else if (t2) {
                if (e4) {
                    var r5 = d[2] || (d[2] = {});
                    "..." === e4.slice(0, 3) ? Object.assign(r5, arguments[++h]) : ([a1, l3] = e4.split("="), r5[g(a1)] = !l3 || g(l3));
                }
            } else {
                for(i21 = g(e4); o1.close[d[1] + i21];)m();
                d = [
                    d,
                    i21,
                    null
...
const xhtm1 = o1;
const validStyleAttribute1 = Object.freeze(new Set([
    "accent-color",
    "align-content",
    "align-items",
    "align-self",
...
const validNonCallbackHtmlAttributes1 = Object.freeze(new Set([
    "class",
    "style",
    "value",
    "id",
...
const isValidStyleAttribute1 = (key)=>key.startsWith('-') || validStyleAttribute1.has(key)
;
...
const attachProperties1 = (source, target)=>{
    const attributes = allKeyDescriptions1(source);
    const propertiesDefition = {};
    for (const [key, value] of Object.entries(attributes)){
        if ([
            'constructor',
...
class ElementalClass1 {
    constructor(components = {}, options = {}){
        const { middleware: middleware3 , errorComponentFactory  } = options || {};
        this.components = components || {};
    ...
    createElement(...args) {
        if (args[0] instanceof Array) {
            ...
            if (properties instanceof Object) {
                for (let [key, value] of Object.entries(properties)){
                    if (key == 'style') {
                        styleString += ElementalClass1.css(value);
                        continue;
                    }
                    if (key.slice(0, 2) == 'on' && value instanceof Function) {
                        element.addEventListener(key.slice(2).toLowerCase(), value);
                    ...
                    if (isSvg) {
                        ...
                        element.setAttribute(kebabCase1(key), value);
                        continue;
                    }
                    if (value != null && !(value instanceof Object) && validNonCallbackHtmlAttributes1.has(key)) {
                        element.setAttribute(key, value);
                    }
                    try {
                        element[key] = value;
                    } catch (error) {}
                    if (isValidStyleAttribute1(key)) {
                        styleString += `;${key}: ${value};`;
                    }
                }
            }
    ...
    extend(additionalComponents = {}, options = {}) {
        const { middleware: middleware4 , ...other } = options || {};
        return Elemental1({
            ...this.components,
            ...additionalComponents
        }, {
...
const Elemental1 = (...args)=>{
    ...
    attachProperties1(elementalObject, createElementFunction);
    return createElementFunction;
};
attachProperties1(ElementalClass1, Elemental1);
function defaultErrorComponentFactory1({ children , ...properties }, key, error6) {
    const element = document.createElement("div");
    const errorDetails = document.createElement("code");
    const childContainer = document.createElement("div");
    element.setAttribute('style', `
...
export { intersection as intersection, subtract as subtract, Column as Column, Row as Row, stringToBytes as stringToBytes, html1 as html,  };
